...
### Changed
- `Mem0Backend.health()` now hits `GET /openapi.json` instead of `GET /memories`. It's a liveness check, not a data-plane check — mem0's store being down no longer flips the `/health` endpoint to unhealthy, and neither does an expired or wrong `MEM0_API_KEY`, since `/openapi.json` carries no auth dependency; real backend and credential failures now surface on real requests instead of at the health gate.
