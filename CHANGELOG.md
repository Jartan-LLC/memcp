...
### Added
- **What cognee costs to run is stated where the upgrade is offered, with its arithmetic.** About $0.50 per 1,000 memory operations — approximately — at OpenAI list prices read 2026-08-18 for the `gpt-4o-mini` and `text-embedding-3-small` pair `memcp up --backend cognee` provisions. The README's upgrade step and `docs/deployment.md` carry the per-operation token counts, counted against cognee 1.5.0's own prompts, the prices, and which parts are estimated (output tokens) versus counted. The same place states that cognee needs an OpenAI-compatible key and that no keyless cognee path is usable on ordinary hardware today. (JAR-632)
- `docs/local-models.md` and `tools/local_model_retrieval.py` characterise the `--llm-base-url` path instead of assuming it. On five CPU cores with Qwen2.5-1.5B-Instruct Q4_K_M, one `add_memory` through cognee drives six to seven model calls and takes three to five minutes, exceeds the adapter's 120s default, and sometimes fails outright when the model cannot produce cognee's extraction schema. `COGNEE_TIMEOUT` exists because of that measurement. (JAR-384)
