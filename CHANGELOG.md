...
### Changed
- `agent_id` scope values are now normalized — lowercased, trimmed, non-`[a-z0-9]` runs collapsed to a single hyphen — identically on every write and filter path (`add_memory`, `search_memory`, `list_memories`, `delete_all_memories`, `memory_entities`, `import_memories`). Server `INSTRUCTIONS` now ask connecting agents to send their own name in that lowercase-hyphenated form (e.g. `claude-code`) as `agent_id`, a change from the opaque-identifier convention agents were using before; existing memories written under that prior convention are unaffected but a normalized filter will no longer reach a non-normalized stored value. (#23)
