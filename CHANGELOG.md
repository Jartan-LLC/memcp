...
### Fixed
- **`memory_entities` on mem0 no longer reads a global endpoint.** It post-filtered mem0's `GET /entities` — which buckets every payload in the store by `user_id`, `agent_id` and `run_id` — on `id == user_id` alone. The rows carry no owner, and `agent_id`/`run_id` are caller-controlled, so one tenant could put a row into another's output by writing under `scope={"agent_id": "<victim's user_id>"}`, and a value two tenants shared reported a `total_memories` and a last-write time covering both. No content, ids or write access crossed. The tool now buckets the caller's own memory listing the way mem0's endpoint buckets the store, so isolation is the query rather than a filter over the answer; rows are unchanged in shape, the caller's `agent`/`run` rows now appear whatever they are named, and counts share the 1000-memory list ceiling. `scope` narrows the derivation instead of being ignored. mem0's three parameter builders also drop `user_id` out of a caller scope rather than trusting the tool layer to have stripped it. (SEC-2026-0065, JAR-457/JAR-460)
